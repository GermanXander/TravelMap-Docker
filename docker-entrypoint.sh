...
sed -i "s/private const DB_HOST = '127.0.0.1';/private const DB_HOST = getenv('DB_HOST') ?: '127.0.0.1';/" /var/www/html/config/db.php
sed -i "s/private const DB_NAME = 'travelmap';/private const DB_NAME = getenv('DB_NAME') ?: 'travelmap';/" /var/www/html/config/db.php
sed -i "s/private const DB_USER = 'root';/private const DB_USER = getenv('DB_USER') ?: 'root';/" /var/www/html/config/db.php
sed -i "s/private const DB_PASS = '';/private const DB_PASS = getenv('DB_PASSWORD') ?: '';/" /var/www/html/config/db.php
